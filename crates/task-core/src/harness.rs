...
pub const BUILTIN_HARNESSES: [&str; 4] = [BUILTIN_CONVERSATION, BUILTIN_PLAN, BUILTIN_REVIEWER, BUILTIN_SMOKE];
...
impl HarnessRegistry {
    ...
    pub fn conversation_id(&self) -> Option<&str> {
        self.harnesses.iter().find(|h| h.conversation).map(|h| h.id.as_str())
    }
    ...
    pub fn from_legacy(
        ...
        let used: Vec<&str> = genres.iter().filter_map(|g| g.default_role.as_deref()).collect();
...
mod tests {
    ...
    fn from_legacy_maps_genres_to_harnesses_and_folds_the_default_role_in() {
        ...
        assert_eq!(secretary.instructions.as_deref(), Some("あなたは人の秘書。"));
    ...
    fn declared_harnesses_override_the_builtins() {
        ...
        assert_eq!(registry.all().iter().filter(|h| h.id == BUILTIN_REVIEWER).count(), 1);
    ...
    fn the_registry_projects_back_to_genres_and_roles() {
        let registry = HarnessRegistry::new(vec![HarnessSpec {
            ...
            adapter: Some("claude-code".into()),
            tier: Some(Tier::Standard),
            budget: HarnessBudget { max_turns: Some(60), max_wall_secs: Some(3600), max_retries: Some(1) },
    ...
    fn known_harness_ids_adds_the_builtins_but_stays_empty_for_a_minimal_config() {
        ...
        let genres = vec![GenreSpec { id: "coding".into(), ..GenreSpec::default() }];
