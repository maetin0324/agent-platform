...
pub fn ancestry<'a>(nodes: &'a [OrgNode], node_id: &str) -> Vec<&'a OrgNode> {
    ...
    while let Some(node) = cursor {
        ...
        cursor = node.parent_id.as_deref().and_then(|p| nodes.iter().find(|n| n.id == p));
...
pub fn resolve(nodes: &[OrgNode], node_id: &str) -> EffectiveProfile {
    ...
    for node in &chain {
        ...
        push_unique(&mut out.harnesses_allowed, p.harnesses.allowed.iter().cloned());
...
pub fn validate_profile(profile: &Profile, known_harnesses: &[String]) -> Result<(), ProfileError> {
    for skill in &profile.skills {
        if !is_valid_skill(skill) {
            return Err(ProfileError::InvalidSkill { skill: skill.clone() });
        }
    }
...
mod tests {
    ...
    fn node(id: &str, parent: Option<&str>, profile: Profile) -> OrgNode {
        ...
        OrgNode {
            ...
            name: id.to_string(),
            kind: if parent.is_none() { OrgKind::Secretary } else { OrgKind::Department },
            genre: None,
            brief: String::new(),
            profile,
            position: 0,
        ...
        vec![
            node(
                ...
                Profile {
                    ...
                    permissions: Permissions { approvals: vec!["external-post".into()] },
            ...
            node(
                ...
                Profile {
                    ...
                    harnesses: HarnessPrefs { allowed: vec!["coding".into()], default: None },
            ...
            node(
                ...
                Profile {
                    ...
                    harnesses: HarnessPrefs { allowed: vec![], default: Some("coding".into()) },
                    deny_tools: vec!["docker".into()],
                    tools: vec!["tavily".into()],
                    policy: vec!["課の方針".into()],
                    model: ModelPrefs { tier: Some(Tier::Cheap), allowed_tiers: vec![] },
                    run: Some(ProfileRun::Container),
                    review: ReviewPrefs { harness: Some("reviewer".into()), tier: Some(Tier::Cheap) },
                    permissions: Permissions { approvals: vec!["cluster-write".into()] },
                    ..Profile::default()
                },
            ),
        ]
    ...
    fn resolve_merges_lists_by_union_scalars_by_child_and_tiers_by_intersection() {
        ...
        assert_eq!(eff.chain, vec!["cos", "engineering", "software-engineering"]);
        // 和（根→葉の順、重複は落ちる）。
        assert_eq!(eff.skills, vec!["coordination", "software", "rust"]);
        assert_eq!(eff.harnesses_allowed, vec!["conversation", "plan", "coding"]);
    ...
    fn an_empty_parent_allowed_tiers_means_no_restriction() {
        let org = vec![
            ...
            node(
                ...
                Some("cos"),
                Profile {
                    model: ModelPrefs { tier: None, allowed_tiers: vec![Tier::Cheap] },
                    ..Profile::default()
                },
            ),
        ];
        assert_eq!(resolve(&org, "engineering").allowed_tiers, vec![Tier::Cheap]);
    ...
    fn resolve_of_an_unknown_node_is_empty_and_a_broken_chain_terminates() {
        ...
        // 親の連鎖が閉じた壊れたデータでも止まる。
        let broken = vec![node("a", Some("b"), Profile::default()), node("b", Some("a"), Profile::default())];
    ...
    fn effective_tools_expose_the_clusters_and_the_allows_helpers() {
        let org = vec![node(
            ...
            None,
            Profile {
                tools: vec!["cluster:pegasus".into(), "cluster:sirius".into(), "gh".into()],
                harnesses: HarnessPrefs { allowed: vec!["coding".into()], default: None },
                ..Profile::default()
            },
        )];
    ...
    fn with_task_overrides_harness_tier_and_skills() {
        ...
        let bare = crate::model::Task { genre: None, skills: vec![], ..sample_task() };
    ...
    fn validate_profile_rejects_unknown_tools_harnesses_and_skills() {
        ...
        let ok = Profile {
            ...
            harnesses: HarnessPrefs { allowed: vec!["coding".into()], default: Some("coding".into()) },
            review: ReviewPrefs { harness: Some("conversation".into()), tier: None },
            ..Profile::default()
        };
        validate_profile(&ok, &known).expect("ok");

        let bad_tool = Profile { tools: vec!["kubectl".into()], ..Profile::default() };
        assert!(matches!(
            validate_profile(&bad_tool, &known),
            Err(ProfileError::UnknownTool { .. })
        ));
        // `cluster:` だけ（id が空）も駄目。
        let empty_cluster = Profile { tools: vec!["cluster:".into()], ..Profile::default() };
        assert!(validate_profile(&empty_cluster, &known).is_err());
        let bad_harness = Profile {
            harnesses: HarnessPrefs { allowed: vec!["ghost".into()], default: None },
            ..Profile::default()
        };
        assert!(matches!(
            validate_profile(&bad_harness, &known),
            Err(ProfileError::UnknownHarness { field: "harnesses.allowed", .. })
        ));
        let bad_review = Profile {
            review: ReviewPrefs { harness: Some("ghost".into()), tier: None },
            ..Profile::default()
        };
        assert!(matches!(
            validate_profile(&bad_review, &known),
            Err(ProfileError::UnknownHarness { field: "review.harness", .. })
        ));
        let bad_skill = Profile { skills: vec!["IO_Uring".into()], ..Profile::default() };
        assert!(matches!(
    ...
    fn an_empty_profile_serializes_to_an_empty_object() {
        ...
        assert_eq!(serde_json::to_string(&Profile::default()).expect("json"), "{}");
