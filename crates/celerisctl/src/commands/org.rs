...
pub fn mapping() -> BTreeMap<String, String> {
    V1_TO_V2.iter().map(|(a, b)| (a.to_string(), b.to_string())).collect()
}
...
fn migrate(
    ...
    if before.is_empty() {
        outln!("org_nodes is empty; nothing to migrate (the seed will be planted on the next start)");
    ...
    outln!("org migrate-v2 (ADR-0046 D7){}", if dry_run { " [dry-run]" } else { "" });
    ...
    for node in &unmapped {
        outln!("  keep   {} (not in the ADR-0046 D7 map; left as is)", node.id);
    }
    if dry_run {
        return Ok(ExitCode::SUCCESS);
    }
    ...
    outln!("  rewrote {} row(s) that referenced an old node id", rows.len());
        ...
        for (from, to) in &renames {
            ...
            if !dest.exists() {
                ...
                std::fs::rename(&src, &dest)
                    .map_err(|e| CliError::msg(format!("move {} -> {}: {e}", src.display(), dest.display())))?;
                memory_moves.push(MemoryMove { from: src, to: dest });
                continue;
            }
            ...
            std::fs::create_dir_all(&saved_root).map_err(|e| CliError::msg(format!("mkdir: {e}")))?;
            let saved = saved_root.join(from);
            std::fs::rename(&src, &saved)
                .map_err(|e| CliError::msg(format!("save {} -> {}: {e}", src.display(), saved.display())))?;
    ...
    for node in &before {
        let new_id = map.get(&node.id).cloned().unwrap_or_else(|| node.id.clone());
        ...
        next.push(OrgNode {
            ...
            name: seeded.map(|s| s.name.clone()).unwrap_or_else(|| node.name.clone()),
            kind: seeded.map(|s| s.kind).unwrap_or(node.kind),
            genre: seeded.and_then(|s| s.genre.clone()).or_else(|| node.genre.clone()),
            brief: match seeded {
    ...
    std::fs::create_dir_all(backup_dir).map_err(|e| CliError::msg(format!("mkdir {}: {e}", backup_dir.display())))?;
...
fn rollback(store: &SqliteStore, backup_dir: &Path, dry_run: bool) -> Result<ExitCode, CliError> {
    ...
    let text = std::fs::read_to_string(&path)
        .map_err(|e| CliError::msg(format!("read {}: {e} (migrate-v2 was never run here?)", path.display())))?;
    let backup: MigrationBackup =
        serde_json::from_str(&text).map_err(|e| CliError::msg(format!("{}: {e}", path.display())))?;
    outln!(
    ...
    for append in &backup.memory_appends {
        ...
        if append.saved_from.is_dir() && !append.original_dir.exists() {
            std::fs::rename(&append.saved_from, &append.original_dir)
                .map_err(|e| CliError::msg(format!("restore {}: {e}", append.original_dir.display())))?;
        }
    }
...
mod tests {
    ...
    fn the_map_is_deterministic_and_merges_the_poc_section() {
        ...
        assert_eq!(map.get("coding-frontend").map(String::as_str), Some("software-engineering"));
        assert_eq!(map.get("coding-poc").map(String::as_str), Some("software-engineering"));
        ...
        assert_eq!(
            map.values().filter(|v| v.as_str() == "software-engineering").count(),
            2,
