...
///
/// TODO(ADR-0043 D2 / A1): `repos`（案件の複数リポジトリからこのタスクが使うものを選ぶ）は
/// ADR-0043 A1 の枝が `Task.repos` と一緒に足す。ここに `repos: Option<Vec<String>>` を 1 行足し、
/// `apply` で `task.repos` に写し、`fields` に `"repos"` を積めばよい。
...
/// `budget` はそのまま残る — 変えたければ同じ `PATCH` で明示的に書く（`docs/gui/api.md` §3.68）。
pub fn edit_task(
    store: &dyn TaskStore,
    id: TaskId,
    edit: TaskEdit,
