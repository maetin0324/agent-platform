...
pub fn assignee_accepts(org: &[OrgNode], assignee: &str, harness: Option<&str>) -> Result<(), String> {
...
mod tests {
    ...
    fn node(id: &str, parent: Option<&str>, allowed: &[&str], default: Option<&str>, skills: &[&str]) -> OrgNode {
    ...
    fn org() -> Vec<OrgNode> {
        vec![
            node("cos", None, &["conversation", "plan"], Some("conversation"), &[]),
            node("engineering", Some("cos"), &["coding"], None, &["software"]),
            node("software-engineering", Some("engineering"), &[], Some("coding"), &["rust", "sqlite"]),
            node("systems-performance", Some("engineering"), &["data-analysis"], None, &["hpc", "benchmark", "rust"]),
        ]
    }
    ...
    fn ties_go_to_the_shallower_node_then_to_the_lexicographically_smaller_id() {
        ...
        let Assignment::Assigned { node, .. } = decide(&org, &task(Some("coding"), &["rust"])) else {
            panic!("assigned");
        };
        assert_eq!(node, "software-engineering");
        // `software` は engineering（深さ 2）だけが持つ。
        let Assignment::Assigned { node, score, .. } = decide(&org, &task(Some("coding"), &["software"])) else {
            panic!("assigned");
        };
    ...
    fn without_skills_the_default_harness_node_wins_then_the_shallowest() {
        let org = org();
        let Assignment::Assigned { node, score, reason } = decide(&org, &task(Some("coding"), &[])) else {
            panic!("assigned");
        };
        ...
        let Assignment::Assigned { node, .. } = decide(&org, &task(Some("data-analysis"), &[])) else {
            panic!("assigned");
        };
    ...
    fn no_candidate_is_unroutable_with_a_question() {
        let org = org();
        let Assignment::Unroutable { question } = decide(&org, &task(Some("literature"), &["paper-writing"])) else {
            panic!("unroutable");
        };
        assert!(question.starts_with("担当が見つからない: harness literature / skills paper-writing"), "{question}");
    }
    ...
    fn an_explicit_assignee_must_allow_the_harness() {
        ...
        let err = assignee_accepts(&org, "software-engineering", Some("literature")).expect_err("rejected");
