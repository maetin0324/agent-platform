...
use llm_proxy::config::{ClaudeOauthConfig, CodexOauthConfig, LlmProxyConfig, OpenAiCompatibleConfig};
...
fn write_claude_credentials(dir: &std::path::Path, account_id: &str, access_token: &str, refresh_token: &str, expires_at_ms: i64) {
    ...
    std::fs::write(account_dir.join(".credentials.json"), serde_json::to_vec_pretty(&value).expect("json")).expect("write");
}

fn write_codex_credentials(dir: &std::path::Path, account_id: &str, access_token: &str, refresh_token: &str) {
    let account_dir = dir.join(account_id);
    std::fs::create_dir_all(&account_dir).expect("mkdir");
    let value = json!({
        "OPENAI_API_KEY": null,
    ...
    std::fs::write(account_dir.join("auth.json"), serde_json::to_vec_pretty(&value).expect("json")).expect("write");
}
...
async fn get_json(client: &reqwest::Client, addr: SocketAddr, path: &str, token: Option<&str>) -> reqwest::Response {
...
async fn post_chat(client: &reqwest::Client, addr: SocketAddr, body: &Value, token: Option<&str>) -> reqwest::Response {
    let mut req = client.post(format!("http://{addr}/v1/chat/completions")).json(body);
    if let Some(t) = token {
        req = req.bearer_auth(t);
    }
    req.send().await.expect("send")
...
async fn anthropic_messages(AxumState(fake): AxumState<Arc<AnthropicFake>>, Json(body): Json<Value>) -> axum::response::Response {
    ...
    let has_tool_result = body["messages"]
        ...
        .map(|ms| {
            ms.iter().any(|m| {
                m["content"].as_array().map(|c| c.iter().any(|b| b["type"] == "tool_result")).unwrap_or(false)
            })
        })
        .unwrap_or(false);
    ...
    if stream {
        let text = sse_body(&[
            ("message_start", json!({"message": {"id": "msg_stream_1", "usage": {"input_tokens": 5}}})),
            ...
            ("message_delta", json!({"delta": {"stop_reason": "end_turn"}, "usage": {"output_tokens": 3}})),
            ("message_stop", json!({})),
        ]);
...
async fn claude_non_stream_round_trip() {
    ...
    assert_eq!(resp.headers().get("x-celeris-source").and_then(|v| v.to_str().ok()), Some("claude-oauth"));
...
async fn claude_tool_call_round_trip() {
    ...
    assert!(call["function"]["arguments"].as_str().expect("args").contains("Tsukuba"));
    ...
    assert_eq!(body2["choices"][0]["message"]["content"], "The weather is sunny.");
...
async fn claude_401_refreshes_and_retries_then_writes_back_the_credentials_file() {
    ...
    async fn handler(AxumState(state): AxumState<Arc<Once401>>, headers: HeaderMap) -> axum::response::Response {
        ...
        assert!(auth.contains("fresh-access"), "expected the refreshed token, got {auth}");
        Json(json!({
            "id": "msg_ok",
    ...
    let accounts_tmp = tempfile::tempdir().expect("tmp");
    write_claude_credentials(accounts_tmp.path(), "acct-a", "old-access", "old-refresh", far_future_ms());
    ...
    let on_disk: Value = serde_json::from_str(&std::fs::read_to_string(accounts_tmp.path().join("acct-a").join(".credentials.json")).expect("read")).expect("json");
...
async fn claude_429_falls_back_to_the_next_account_and_records_a_cooldown() {
    ...
    async fn first_429_then_ok(AxumState(counter): AxumState<Arc<AtomicU32>>) -> axum::response::Response {
    ...
    let app = Router::new().route("/v1/messages", post(first_429_then_ok)).with_state(counter);
    ...
    let accounts_tmp = tempfile::tempdir().expect("tmp");
    write_claude_credentials(accounts_tmp.path(), "acct-a", "fake-access-a", "fake-refresh-a", far_future_ms());
    write_claude_credentials(accounts_tmp.path(), "acct-b", "fake-access-b", "fake-refresh-b", far_future_ms());

    let mut config = base_config();
    config.sources.claude_oauth = Some(claude_source(upstream_addr, accounts_tmp.path().to_path_buf()));
    let book = Arc::new(StdMutex::new(AccountBook::new_in_memory()));
    let state = ProxyState::new(config, reqwest::Client::new(), Some(book.clone()), None, None, SharedRole::default(), None, std::time::Duration::from_secs(5));
    let (addr, _h4) = spawn(router(state)).await;
    let client = reqwest::Client::new();

    let resp = post_chat(&client, addr, &chat_request("claude/standard", false), None).await;
    assert_eq!(resp.status(), StatusCode::OK);
    assert_eq!(resp.headers().get("x-celeris-account").and_then(|v| v.to_str().ok()), Some("acct-b"));
    let body: Value = resp.json().await.expect("json");
    assert_eq!(body["choices"][0]["message"]["content"], "second account succeeded");
...
async fn codex_non_stream_round_trip() {
    ...
    assert_eq!(resp.status(), StatusCode::OK);
    assert_eq!(resp.headers().get("x-celeris-source").and_then(|v| v.to_str().ok()), Some("codex-oauth"));
    let body: Value = resp.json().await.expect("json");
    assert_eq!(body["choices"][0]["finish_reason"], "tool_calls");
...
async fn codex_tool_call_round_trip() {
    ...
    let resp2 = post_chat(&client, addr, &second, None).await;
    let body2: Value = resp2.json().await.expect("json");
    assert_eq!(body2["choices"][0]["message"]["content"], "It is sunny in Tsukuba.");
}
...
async fn relay_models() -> axum::response::Response {
    Json(json!({"object": "list", "data": [{"id": "qwen3.8-27b", "object": "model"}]})).into_response()
}
...
fn relay_router() -> Router {
    Router::new().route("/v1/models", get(relay_models)).route("/v1/chat/completions", post(relay_chat))
}
...
async fn relay_non_stream_passes_through_and_rewrites_the_model_field() {
    ...
    let mut config = base_config();
    config.sources.openai_compatible.push(OpenAiCompatibleConfig {
        id: "qwen".into(),
        base_url: format!("http://{upstream_addr}/v1"),
        api_key: None,
        enabled: true,
    });
    let state = ProxyState::new(config, reqwest::Client::new(), None, None, None, SharedRole::default(), None, std::time::Duration::from_secs(5));
    let (addr, _h2) = spawn(router(state)).await;
    let client = reqwest::Client::new();
    let resp = post_chat(&client, addr, &chat_request("qwen/cheap", false), None).await;
    assert_eq!(resp.status(), StatusCode::OK);
    assert_eq!(resp.headers().get("x-celeris-source").and_then(|v| v.to_str().ok()), Some("openai-compatible:qwen"));
    let body: Value = resp.json().await.expect("json");
...
async fn celeris_tier_prefers_the_reachable_relay_when_prefer_free() {
    ...
    config.sources.claude_oauth = Some(claude_source(anthropic_addr, accounts_tmp.path().to_path_buf()));
    config.sources.openai_compatible.push(OpenAiCompatibleConfig {
        id: "qwen".into(),
...
async fn bearer_is_required_when_a_token_is_configured() {
    let state = ProxyState::new(base_config(), reqwest::Client::new(), None, None, Some("s3cr3t".to_string()), SharedRole::default(), None, std::time::Duration::from_secs(5));
...
async fn embeddings_are_not_implemented() {
    let state = ProxyState::new(base_config(), reqwest::Client::new(), None, None, None, SharedRole::default(), None, std::time::Duration::from_secs(5));
    let (addr, _h) = spawn(router(state)).await;
    let client = reqwest::Client::new();
    let resp = client
...
async fn models_lists_only_the_configured_sources() {
    ...
    assert_eq!(resp.status(), StatusCode::OK);
    let body: Value = resp.json().await.expect("json");
    let ids: Vec<String> = body["data"].as_array().expect("data").iter().map(|m| m["id"].as_str().unwrap_or_default().to_string()).collect();
...
async fn standby_returns_503_like_other_admin_endpoints() {
    ...
    let state = ProxyState::new(base_config(), reqwest::Client::new(), None, None, None, role, None, std::time::Duration::from_secs(5));
    let (addr, _h) = spawn(router(state)).await;
    let client = reqwest::Client::new();
    let resp = get_json(&client, addr, "/v1/models", None).await;
...
async fn sources_view_reports_cooldown_and_hourly_counts() {
    ...
    book.lock()
        .expect("lock")
        .set_cooldown("acct-a", task_dispatch::accounts::AccountCooldown { until: now + 500, reason: task_dispatch::accounts::AccountCooldownReason::Throttled }, now);
    let state = llm_proxy::ProxyState::new(config, reqwest::Client::new(), Some(book), None, None, SharedRole::default(), Some(db_path), std::time::Duration::from_secs(5));
    let view = state.sources_view(now).await;
    let claude = view.sources.iter().find(|s| s.id == "claude-oauth").expect("claude source");
    let acct = claude.accounts.iter().find(|a| a.id == "acct-a").expect("acct-a");
...
async fn no_secret_value_appears_in_the_logs_even_across_a_token_refresh() {
    async fn handler(headers: HeaderMap, calls: AxumState<Arc<AtomicU32>>) -> axum::response::Response {
    ...
    let app = Router::new().route("/v1/messages", post(handler)).with_state(calls).route("/oauth/token", post(refresh));
    let (upstream_addr, _h1) = spawn(app).await;

    let accounts_tmp = tempfile::tempdir().expect("tmp");
    write_claude_credentials(accounts_tmp.path(), "acct-a", "super-secret-original-access", "super-secret-original-refresh", far_future_ms());

    let mut config = base_config();
    config.sources.claude_oauth = Some(claude_source(upstream_addr, accounts_tmp.path().to_path_buf()));
    let state = ProxyState::new(config, reqwest::Client::new(), Some(Arc::new(StdMutex::new(AccountBook::new_in_memory()))), None, None, SharedRole::default(), None, std::time::Duration::from_secs(5));
    ...
    for secret in ["super-secret-original-access", "super-secret-original-refresh", "brand-new-secret-access", "brand-new-secret-refresh"] {
