...
pub struct Config {
    /// ADR-0065 D1: 従来どおり `db = "<path>"`（文字列）で書けるほか、`[db]` テーブルで
...
/// ADR-0065 D1: `db` キー。**文字列**（`db = "<path>"`。従来どおり、互換）か、**テーブル**
...
impl Config {
    ...
    pub fn load(path: &Path) -> Result<Self, ConfigError> {
        ...
        // ADR-0045 D2 / ADR-0065 D1: `db` の既定は `~/.local/celeris/celeris.sqlite3`。`~` を展開して
...
"#;
    ...
    /// ADR-0065 D1: `db` は従来どおり文字列（`db = "<path>"`）でも、`[db]` テーブル
