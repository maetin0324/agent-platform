//! ADR-0065 D3 / D5（Phase 110a）: DB の背景メンテナンス。
//!
...
/// `background_checkpoint` の接続に合わせる WAL の目安サイズ（64 MB。ADR-0065 D5）。
...
/// ADR-0065 D5: `checkpoint_interval` ごとに `PRAGMA wal_checkpoint(PASSIVE)` を打つ背景タスクを起こす。
...
fn checkpoint_once(db_path: &Path, busy_timeout: Duration) -> Result<(), rusqlite::Error> {
    ...
    // ADR-0065 D5: WAL が `journal_size_limit`（64 MB）を超えていたら TRUNCATE も試みる（PASSIVE は
...
/// ADR-0065 D3: `backup_interval` ごとに `backup_dir` へ `celeris-<unix_ts>.sqlite3` を書き、
...
/// ADR-0065 D3: 1 回分のバックアップ。書いたファイルのパスを返す。
fn backup_once(
    db_path: &Path,
    backup_dir: &Path,
    keep: usize,
