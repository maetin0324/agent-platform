...
impl AccountBook {
    ...
    pub fn load(path: &Path) -> Self {
        let mut book = Self { states: BTreeMap::new(), path: Some(path.to_path_buf()) };
    ...
    pub fn record_observation(&mut self, id: &str, obs: RateLimitObservation, source: ObservationSource) {
        let state = self.states.entry(id.to_string()).or_default();
        let newer = state.usage.as_ref().is_none_or(|u| obs.observed_at >= u.observed_at);
        if newer {
    ...
    pub fn set_cooldown(&mut self, id: &str, cooldown: AccountCooldown, now: i64) {
        ...
        let next = match state.cooldown {
            ...
            Some(existing) if cooldown.reason == AccountCooldownReason::AuthFailed => {
                AccountCooldown { until: cooldown.until.max(existing.until), reason: AccountCooldownReason::AuthFailed }
            }
            Some(existing) if existing.reason == AccountCooldownReason::AuthFailed && existing.until > now => {
                AccountCooldown { until: cooldown.until.max(existing.until), reason: AccountCooldownReason::AuthFailed }
            }
    ...
    pub fn save(&self) -> std::io::Result<()> {
        ...
        let file = BookFile { version: 1, accounts: self.states.clone() };
...
fn effective_utilization(w: RateWindow, now: i64) -> f64 {
    if now < w.resets_at { w.utilization } else { 0.0 }
}
...
pub fn cooldown_for_failure(
    ...
    if matches!(reason, AccountCooldownReason::AuthFailed) {
        return AccountCooldown { until: fallback_until, reason };
    }
    ...
    AccountCooldown { until: latest.unwrap_or(fallback_until), reason }
}
...
pub fn evaluate(c: &AccountCandidate<'_>, state: Option<&AccountState>, max_runs_per_account: usize, now: i64) -> AccountEvaluation {
    let id = c.id.to_string();
    let excluded = |reason: ExcludedReason| AccountEvaluation { id: id.clone(), score: None, excluded: Some(reason) };

    if !c.logged_in {
        return excluded(ExcludedReason::NotLoggedIn);
    }
    if state.and_then(|s| s.cooldown.as_ref()).is_some_and(|cd| cd.until > now) {
    ...
    let score = if obs.is_none() {
        ...
        let h7 = match seven_day {
            ...
            Some(w) if now < w.resets_at => {
                let t7 = ((w.resets_at - now) as f64 / SEVEN_DAY_SECS as f64).max(MIN_WEEK_FRACTION);
                ((1.0 - u7) / t7).min(1.0)
            }
            _ => 1.0 - u7,
        };
    ...
    AccountEvaluation { id, score: Some(score), excluded: None }
}

/// スコア最大のアカウントを選ぶ。同点は `in_use` の少ない方、次に `id` の昇順（ADR-0024 D3-4）。
/// 選べる候補が無ければ `None`。
pub fn select_account(cands: &[AccountCandidate<'_>], book: &AccountBook, max_runs_per_account: usize, now: i64) -> Option<String> {
    ...
    for c in cands {
        ...
        let take = match &best {
            None => true,
            Some((best_id, best_score, best_in_use)) => match score.partial_cmp(best_score).unwrap_or(std::cmp::Ordering::Equal) {
...
mod tests {
    ...
    fn window(utilization: f64, resets_at: i64) -> RateWindow {
        RateWindow { utilization, resets_at }
    }

    fn obs(five_hour: Option<RateWindow>, seven_day: Option<RateWindow>, observed_at: i64) -> RateLimitObservation {
        RateLimitObservation { five_hour, seven_day, status: None, resets_at: None, observed_at }
    }

    fn state_with_usage(usage: RateLimitObservation) -> AccountState {
        AccountState { usage: Some(usage), ..Default::default() }
    }

    fn state_with_cooldown(cooldown: AccountCooldown) -> AccountState {
        AccountState { cooldown: Some(cooldown), ..Default::default() }
    }
    ...
    fn scan_accounts_finds_valid_dirs_sorted_with_login_state() {
        ...
        assert_eq!(
            found,
            vec![
                AccountDir { id: "alpha".into(), dir: root.join("alpha"), logged_in: false },
                AccountDir { id: "bravo".into(), dir: root.join("bravo"), logged_in: true },
            ]
        );
    }
    ...
    fn book_save_and_load_round_trip() {
        ...
        book.record_observation("a", obs(Some(window(0.5, 100)), None, 10), ObservationSource::Run);
        book.set_cooldown("a", AccountCooldown { until: 200, reason: AccountCooldownReason::Throttled }, 0);
        book.record_check("b", AccountCheckRecord { at: 5, result: "ok".into(), detail: None });
        ...
        assert_eq!(reloaded.state("a").expect("a").usage.as_ref().expect("usage").observed_at, 10);
        assert_eq!(reloaded.state("a").expect("a").cooldown.as_ref().expect("cd").until, 200);
        assert_eq!(reloaded.state("b").expect("b").last_check.as_ref().expect("chk").result, "ok");
    }
    ...
    fn book_load_missing_file_starts_empty_but_remembers_path() {
        ...
        book.record_check("a", AccountCheckRecord { at: 1, result: "ok".into(), detail: None });
    ...
    fn book_record_observation_keeps_newest_by_observed_at() {
        let mut book = AccountBook::new_in_memory();
        book.record_observation("a", obs(Some(window(0.1, 100)), None, 10), ObservationSource::Run);
        book.record_observation("a", obs(Some(window(0.9, 200)), None, 5), ObservationSource::Check); // older: ignored
        assert_eq!(book.state("a").expect("a").usage.as_ref().expect("u").observed_at, 10);
        assert_eq!(book.state("a").expect("a").source, Some(ObservationSource::Run));

        book.record_observation("a", obs(Some(window(0.5, 300)), None, 20), ObservationSource::Check); // newer: replaces
        assert_eq!(book.state("a").expect("a").usage.as_ref().expect("u").observed_at, 20);
        assert_eq!(book.state("a").expect("a").source, Some(ObservationSource::Check));
    }
    ...
    fn book_record_observation_ties_prefer_the_incoming_one() {
        let mut book = AccountBook::new_in_memory();
        book.record_observation("a", obs(Some(window(0.1, 100)), None, 10), ObservationSource::Run);
        book.record_observation("a", obs(Some(window(0.9, 200)), None, 10), ObservationSource::Check); // same observed_at: wins
        assert_eq!(book.state("a").expect("a").usage.as_ref().expect("u").five_hour.map(|w| w.utilization), Some(0.9));
        assert_eq!(book.state("a").expect("a").source, Some(ObservationSource::Check));
    }
    ...
    fn book_set_cooldown_keeps_the_later_until_for_non_auth_failed_reasons() {
        let mut book = AccountBook::new_in_memory();
        book.set_cooldown("a", AccountCooldown { until: 100, reason: AccountCooldownReason::Throttled }, 0);
        book.set_cooldown("a", AccountCooldown { until: 50, reason: AccountCooldownReason::Exhausted }, 0); // earlier: ignored
        let cd = book.state("a").expect("a").cooldown.as_ref().expect("cd");
        assert_eq!((cd.until, cd.reason), (100, AccountCooldownReason::Throttled));
        book.set_cooldown("a", AccountCooldown { until: 150, reason: AccountCooldownReason::Exhausted }, 0); // later: replaces
        let cd = book.state("a").expect("a").cooldown.as_ref().expect("cd");
    ...
    fn book_set_cooldown_auth_failed_always_overwrites_the_reason() {
        ...
        book.set_cooldown("a", AccountCooldown { until: 50, reason: AccountCooldownReason::AuthFailed }, 0);
        let cd = book.state("a").expect("a").cooldown.as_ref().expect("cd");
    ...
    fn book_set_cooldown_keeps_unexpired_auth_failed_reason_when_throttled_follows() {
        let mut book = AccountBook::new_in_memory();
        book.set_cooldown("a", AccountCooldown { until: 1_000, reason: AccountCooldownReason::AuthFailed }, 0);
        // now(0) < until(1_000): まだ失効していない。
        book.set_cooldown("a", AccountCooldown { until: 2_000, reason: AccountCooldownReason::Throttled }, 0);
        let cd = book.state("a").expect("a").cooldown.as_ref().expect("cd");
        assert_eq!(cd.reason, AccountCooldownReason::AuthFailed);
    ...
    fn book_set_cooldown_expired_auth_failed_does_not_block_new_reason() {
        ...
        book.set_cooldown("a", AccountCooldown { until: 3_000, reason: AccountCooldownReason::Throttled }, 2_000);
        let cd = book.state("a").expect("a").cooldown.as_ref().expect("cd");
    ...
    fn book_clear_expired_removes_only_past_cooldowns() {
        let mut book = AccountBook::new_in_memory();
        book.set_cooldown("a", AccountCooldown { until: 100, reason: AccountCooldownReason::Throttled }, 0);
        book.set_cooldown("b", AccountCooldown { until: 300, reason: AccountCooldownReason::Throttled }, 0);
    ...
    fn cooldown_for_failure_auth_failed_uses_fallback() {
        ...
        assert_eq!(cd, AccountCooldown { until: 1_060, reason: AccountCooldownReason::AuthFailed });
    }
    ...
    fn cooldown_for_failure_uses_latest_exhausted_window_reset() {
        let state = state_with_usage(obs(Some(window(0.99, 1_500)), Some(window(0.98, 3_000)), 900));
    ...
    fn cand(id: &str, logged_in: bool, in_use: usize) -> AccountCandidate<'_> {
        AccountCandidate { id, logged_in, in_use }
    }
    ...
    fn evaluate_rejected_without_resets_at_uses_five_hour_window_as_fallback() {
        ...
        let recent = evaluate(&cand("a", true, 0), Some(&state), 2, 1_000 + FIVE_HOUR_SECS - 1);
        assert_eq!(recent.excluded, Some(ExcludedReason::Rejected));

        // observed long ago: no longer treated as rejected
        let stale = evaluate(&cand("a", true, 0), Some(&state), 2, 1_000 + FIVE_HOUR_SECS + 1);
    ...
    fn evaluate_weekly_pacing_is_not_capped_beyond_one_week() {
        ...
        assert!((e_beyond.score.unwrap() - 0.25).abs() < 1e-9, "{:?}", e_beyond.score);
    }
    ...
    fn evaluate_cooldown_excludes_until_expiry() {
        let state = state_with_cooldown(AccountCooldown { until: 2_000, reason: AccountCooldownReason::Throttled });
    ...
    fn select_account_deterministic_tie_break_by_in_use_then_id() {
        ...
        let cands = vec![cand("zebra", true, 0), cand("alpha", true, 0), cand("mid", true, 0)];
        assert_eq!(select_account(&cands, &book, 5, 1_000), Some("alpha".to_string()));

        // Equal scores by observation, but different in_use -> fewer in_use wins.
        let mut book2 = AccountBook::new_in_memory();
        book2.record_observation("a", obs(Some(window(0.2, 9_000)), None, 1), ObservationSource::Run);
        book2.record_observation("b", obs(Some(window(0.2, 9_000)), None, 1), ObservationSource::Run);
        let cands2 = vec![cand("a", true, 1), cand("b", true, 0)];
        assert_eq!(select_account(&cands2, &book2, 5, 1_000), Some("b".to_string()));
    }

    #[test]
    fn select_account_all_excluded_returns_none() {
        let mut book = AccountBook::new_in_memory();
        book.set_cooldown("a", AccountCooldown { until: 5_000, reason: AccountCooldownReason::Throttled }, 0);
    ...
    fn select_account_picks_highest_score_among_mixed_states() {
        let mut book = AccountBook::new_in_memory();
        book.record_observation("nearly-full", obs(Some(window(0.9, 9_000)), None, 1), ObservationSource::Run);
        book.set_cooldown("cooling", AccountCooldown { until: 9_999, reason: AccountCooldownReason::Throttled }, 0);
        // "fresh" has no observation -> score 1.0, the best.
        let cands = vec![cand("nearly-full", true, 0), cand("cooling", true, 0), cand("fresh", true, 0)];
        assert_eq!(select_account(&cands, &book, 5, 1_000), Some("fresh".to_string()));
    }
}
