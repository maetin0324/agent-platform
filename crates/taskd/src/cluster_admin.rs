...
///
/// `auth = "manual"` は「taskd は接続を張らない」ままなので `NotSupported`（409）。
...
/// ADR-0032 D5: `DELETE /clusters/{id}/connect`。進行中のセッションを取り消し、taskd が持っている
...
pub fn spawn_connect_cancel(
    ...
    tokio::spawn(async move {
        ...
        // taskd が保持している master を落とす（Drop でプロセスグループごと SIGKILL）。
...
fn hold_master(
    ...
    let Some(master) = master else {
        // 人が張った master を見つけた場合。taskd の持ち物ではないので登録しない。
        return;
    };
    match masters.lock() {
        Ok(mut held) => {
...
mod tests {
    ...
    /// ADR-0032 D1/D5: `auth = "manual"` は「taskd は接続を張らない」ままなので 409 相当。
    #[tokio::test]
