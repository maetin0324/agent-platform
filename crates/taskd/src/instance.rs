//! ADR-0040 D4（Phase 47）: taskd の「インスタンスの役割」とライブ引き継ぎ。
//!
...
pub const RELEASE_ENV: &str = "TASKD_RELEASE";
...
impl InstanceIdentity {
    /// `release` は `--release <sha12>` > 環境変数 `TASKD_RELEASE` > `"dev"` の順。
...
/// `--release` > `TASKD_RELEASE` > `"dev"`（どちらも空文字は「無い」扱い）。
...
mod tests {
    ...
    /// `release` は `--release` > `TASKD_RELEASE` > `"dev"`。
    #[test]
