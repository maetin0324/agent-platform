...
- 関連: [ADR-GUI-0001 境界](adr/0001-architecture-boundary.md) / [ADR-GUI-0002 フロントエンド](adr/0002-frontend-stack.md) / [taskd HTTP API v1](api.md) /
  [taskd への提案](taskd-proposals.md) / [`taskd-gui` 立ち上げ用ファイル](bootstrap/README.md) / taskd 側 `docs/adr/0013-taskd-api-and-gui-foundations.md`
- `taskd-gui` リポジトリでは本文書が `docs/DESIGN.md`、`api.md` が `docs/taskd-api-v1.md` になる（bootstrap/README.md の対応表）。
...
G フェーズは「taskd API v1 は `api.md` のとおり実装済み」を前提にしてよい。ただし細部が変わりうるので、**G0 で `GET /api/v1/health` とスキーマファイルの存在を確認**する（§10.0）。
...
- 仕様の**曖昧さ**（どちらとも読める）は、taskd の実際の挙動に合わせて進めてよい。ただし PROGRESS の「提案」に「`api.md` の §x をこう明確化すべき」と書く。
...
- taskd API v1: [`api.md`](api.md)。提案の状態: [`taskd-proposals.md`](taskd-proposals.md)
- 一次情報の出典一覧（Remix の実体、ライブラリの版・日付・ライセンス）: ADR-GUI-0002 §2, §8
- `taskd-gui` 立ち上げ: [`bootstrap/README.md`](bootstrap/README.md)
