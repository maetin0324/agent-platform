...
| 57 | PATCH | `/tasks/{id}` | タスクを編集する（題名・目的・受け入れ条件・優先度・ラベル・種類・担当・役割・tier・アダプタ・途中目標・依存・予算。ADR-0044 D1、Phase 53）（**管理系**） | 200 `TaskEditResult` | `task_ops::edit::edit_task` |
| 58 | GET | `/tasks/{id}/comments` | そのタスクのコメント（古い順。ADR-0044 D2） | `CommentList` | store `comments_for` |
| 59 | POST | `/tasks/{id}/comments` | 人のコメント。状態に応じて**割り込み**・回答・記録になる（**管理系**） | 201 `CommentResult` | `task_ops::comment::post_human_comment` |
| 60 | POST | `/tasks/{id}/reopen` | 終端のタスクを同じ worktree のまま再開する（`done`/`failed` → `ready`）（**管理系**） | 200 `TransitionResult` | `task_ops::comment::reopen` |
| 61 | GET | `/tasks/{id}/timeline` | 起きたこと 1 本（イベント・コメント・認可・報告・委譲・リリース。ADR-0044 D5） | `Timeline` | store + `[selfdeploy] releases_dir` |
...
### 3.68〜3.72 タスク管理: 編集・コメント・再開・タイムライン（ADR-0044 B1、Phase 53）
...
#### 3.68 `PATCH /tasks/{id}` → 200 `{task, fields}`（**管理系**）
...
{"title":"…","objective":"…","acceptance":[{"type":"human","text":"…"}],
 "priority":"P1","labels":["infra"],"category":"ops",
...
- 応答の `fields` は**実際に変わった項目の名前**（決まった並び: `title`, `objective`, `acceptance`,
  `priority`, `labels`, `category`, `assignee`, `role`, `tier`, `adapter`, `milestone_id`,
...
- `repos`（ADR-0043 D2）は ADR-0043 A1 が同じ本文に足す（この Phase には無い）。

#### 3.69 `GET /tasks/{id}/comments` → 200 `CommentList`
...
#### 3.70 `POST /tasks/{id}/comments` → 201 `CommentResult`（**管理系**）
...
#### 3.71 `POST /tasks/{id}/reopen` → 200 `TransitionResult`（**管理系**）
...
#### 3.72 `GET /tasks/{id}/timeline` → 200 `Timeline`
...
`approve`: `status == draft` または `kind == approval && status == ready`。`reject`: `kind == approval && status == ready`。`answer`: `status == blocked`。`cancel`: 非終端。`retry`（Phase 31。§3.63）: `status == failed` または `status == cancelled`。**`edit`（ADR-0044 D1。§3.68）: 非終端。`reopen`（ADR-0044 D2。§3.71）: `status == done` または `status == failed`**（`cancelled` には付かない）。
...
pub struct TaskEdit {
    ...
    pub category: Option<TaskCategory>, pub assignee: Option<Option<String>>, pub role: Option<Option<String>>,
