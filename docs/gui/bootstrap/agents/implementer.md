...
---
あなたは celeris-gui プロジェクトの実装担当です。docs/DESIGN.md の設計と docs/celeris-api-v1.md の契約に従います。
...
- celeris の API は docs/celeris-api-v1.md と app/celeris/types.ts にあるものだけを使う。無いものが要ると分かったら、勝手に代替せず「判断が必要な点」として報告に書いて終了する
- SQLite を開かない。ブラウザから celeris を呼ぶコードを書かない。`dangerouslySetInnerHTML` を使わない
