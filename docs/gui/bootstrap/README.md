...
| （新規作成） | `docs/taskd-requests.md` | 空ファイル（見出しだけ）。BLOCKED 時に追記される |
...
`api.md` → `taskd-api-v1.md`、`bootstrap/README.md` → （削除または本ファイルへの参照を外す）。
...
| cargo / rustc | stable（taskd は 1.94、edition 2024） | `cargo --version` | `scripts/taskd.sh build` が `cargo build -p taskd -p taskctl` を実行する |
| taskd | Phase 9a / 9b 完了（`docs/adr/0013`） | `test -f "$TASKD_REPO/docs/api/v1/api-v1.schema.json"` | 無ければ G0 は BLOCKED |
...
環境変数: `TASKD_REPO`（既定 `../agent-platform`）、`TASKD_API_URL`（既定 `http://127.0.0.1:7710`）、`TASKD_API_TOKEN_FILE`（G5）、`TASKD_GUI_BIND`（既定 `127.0.0.1:7700`）、
`TASKD_GUI_PASSWORD_FILE` / `TASKD_GUI_ALLOWED_HOSTS` / `TASKD_GUI_SESSION_SECRET_FILE`（G5）。

## 4. G フェーズの一覧（詳細は `docs/DESIGN.md` §10）

| フェーズ | 内容 | モデル | 最大ターン |
|---|---|---|---|
| G0 | 雛形（React Router 8 framework mode、Biome、Vitest、Playwright、Tailwind、shadcn）、`scripts/taskd.sh`、型生成、`TaskdClient`、Host 検査、taskd 停止時のバナー | strong | 60 |
...
- `## Phase G<N> — BLOCKED`: taskd の API が足りない / 仕様と違う（`docs/taskd-requests.md` に内容）、または同じ失敗が 3 回 + 別案 1 回失敗。オーケストレータが taskd 側を直すか判断してから再開する。
