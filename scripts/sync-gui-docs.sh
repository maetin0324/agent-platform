#!/usr/bin/env bash
# celeris が持つ API 仕様（docs/gui/api.md）を GUI 側の写し（gui/docs/celeris-api-v1.md）に反映する（ADR-0020 D4）。
#
...
tmp="$(mktemp)"
trap 'rm -f "$tmp"' EXIT
# 写し先ではファイル名が celeris-api-v1.md になる（docs/gui/bootstrap/README.md §1 の対応表）。
sed -E 's#\bapi\.md\b#celeris-api-v1.md#g' "$SRC" > "$tmp"
