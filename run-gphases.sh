...
# 使い方: tmux 内で  ./run-gphases.sh                  （G0〜G5）
...
PHASES="${PHASES:-G0 G1 G2 G3 G4 G5}"
MAX_RETRIES="${MAX_RETRIES:-3}"
RETRY_SLEEP="${RETRY_SLEEP:-900}"        # 失敗後の待ち（秒）。レート制限回復待ちを兼ねる
LIGHT_MODEL="${LIGHT_MODEL:-sonnet}"      # G1, G3, G4 のメインセッション
...
maxturns_for() { case "$1" in G0|G2|G5) echo 60;; G4) echo 40;; *) echo 50;; esac; }
