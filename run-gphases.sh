#!/usr/bin/env bash
# celeris-gui 自動進行ランナー（G フェーズ）。run-phases.sh と同じく、フェーズごとに新しい claude -p セッションで /goal を回す。
# GUI 本体は同じリポジトリの gui/（ADR-0020。既定 $CELERIS_REPO/gui）。無ければ docs/gui/ から立ち上げる（docs/gui/bootstrap/README.md）。
# 使い方: tmux 内で  ./run-gphases.sh                  （G0〜G7）
#                   PHASES="G1 G2" ./run-gphases.sh     （一部だけ）
#                   BOOTSTRAP_ONLY=1 ./run-gphases.sh   （前提確認と gui/ の立ち上げだけ）
set -uo pipefail

CELERIS_REPO="$(cd "$(dirname "$0")" && pwd)"
export CELERIS_REPO
GUI_REPO="${GUI_REPO:-$CELERIS_REPO/gui}"   # ADR-0020: celeris と同じリポジトリの gui/
...
LOGDIR="$CELERIS_REPO/logs/gphases"; mkdir -p "$LOGDIR"   # logs/ は .gitignore 済み
...
export CELERIS_API_URL="${CELERIS_API_URL:-http://127.0.0.1:7710}"
export CELERIS_GUI_BIND="${CELERIS_GUI_BIND:-127.0.0.1:7700}"
...
preflight() {
  ...
  if ! grep -q "^## Phase 9 — DONE" "$CELERIS_REPO/docs/PROGRESS.md" 2>/dev/null; then
    log "preflight: celeris の Phase 9（API 層）が DONE になっていない"; missing=1
  fi
  if [ ! -f "$CELERIS_REPO/docs/api/v1/api-v1.schema.json" ]; then
    log "preflight: $CELERIS_REPO/docs/api/v1/api-v1.schema.json が無い（G0 が BLOCKED になる）"; missing=1
  fi
...
bootstrap() {
  ...
  local src="$CELERIS_REPO/docs/gui"
  ...
  cp "$src/api.md"                      "$GUI_REPO/docs/celeris-api-v1.md" &&
  cp "$src"/adr/*.md                    "$GUI_REPO/docs/adr/" &&
  cp "$src/celeris-proposals.md"          "$GUI_REPO/docs/celeris-proposals.md" &&
  ...
  # docs/DESIGN.md のリンクをコピー先に合わせる: api.md → celeris-api-v1.md、bootstrap/README.md は celeris リポジトリ側を指す文に。
  sed -i -E \
    -e 's#\[`bootstrap/README\.md`\]\(bootstrap/README\.md\)#celeris リポジトリの `docs/gui/bootstrap/README.md`#g' \
    -e 's#\[([^]]*)\]\(bootstrap/README\.md\)#\1（celeris リポジトリの docs/gui/bootstrap/README.md）#g' \
    -e 's#(^|[^/])bootstrap/README\.md#\1celeris リポジトリの docs/gui/bootstrap/README.md#g' \
    -e 's#\bapi\.md\b#celeris-api-v1.md#g' \
    "$GUI_REPO/docs/DESIGN.md" || die "bootstrap: link rewrite failed"

  cat > "$GUI_REPO/docs/celeris-requests.md" <<'EOF'
# celeris への依頼

GUI 側で回避せず、celeris の API に足りない・仕様（`docs/celeris-api-v1.md`）と違う点を書く。書いたら `docs/PROGRESS.md` に `## Phase G<N> — BLOCKED` を書いて止まる。
...
EOF
  ...
  # ADR-0020: gui/ が celeris と同じリポジトリの中なら、そこにコミットする（git init はしない）。外を指すなら従来どおり独立したリポジトリにする。
  case "$GUI_REPO/" in
    "$CELERIS_REPO"/*) ( cd "$CELERIS_REPO" && git add -A "$GUI_REPO" && git commit -q -m "bootstrap: gui design, api spec, rules" ) \
                       || die "bootstrap: initial commit failed"
                     log "bootstrap: initial commit $(git -C "$CELERIS_REPO" rev-parse --short HEAD)" ;;
...
for N in $PHASES; do
  ...
  # ADR-0020 D4: celeris が正の API 仕様を GUI 側の写しに反映してからフェーズに入る。
  "$CELERIS_REPO/scripts/sync-gui-docs.sh" >> "$LOGDIR/runner.log" 2>&1 || log "warning: sync-gui-docs.sh failed"
  GOAL="$(sed -e "s/__N__/${N#G}/g" -e "s/__MAXTURNS__/$(maxturns_for "$N")/g" docs/GOAL_TEMPLATE.md)"
  MODEL="$(model_for "$N")"
  attempt=0
  while (( attempt < MAX_RETRIES )); do
    attempt=$((attempt + 1))
    # 前回の celeris（fake ワーカー + [api]）が残っていたら止める。G0 の前は scripts/celeris.sh が無いので何もしない。
    if [ -x scripts/celeris.sh ]; then scripts/celeris.sh stop dev >/dev/null 2>&1 || true; fi
    ...
    if phase_stopped "$N"; then log "phase $N: BLOCKED/PARTIAL — stopping runner (docs/celeris-requests.md を確認)"; exit 2; fi
