...
- 関連: [ADR-GUI-0001 境界](adr/0001-architecture-boundary.md) / [ADR-GUI-0002 フロントエンド](adr/0002-frontend-stack.md) / [celeris HTTP API v1](celeris-api-v1.md) /
  [celeris への提案](celeris-proposals.md) / `celeris-gui` 立ち上げ用ファイル（celeris リポジトリの docs/gui/bootstrap/README.md） / celeris 側 `docs/adr/0013-celeris-api-and-gui-foundations.md`
- `celeris-gui` リポジトリでは本文書が `docs/DESIGN.md`、`celeris-api-v1.md` が `docs/celeris-api-v1.md` になる（celeris リポジトリの docs/gui/bootstrap/README.md の対応表）。
...
| `task-api` | axum。`/api/v1`（`celeris-api-v1.md` の 25 エンドポイント）。`[api]` で有効化。Bearer / Host 検査 / パス検査。`DaemonSnapshot` を `watch` で受ける |
| スキーマ | `docs/api/v1/api-v1.schema.json`（1 ファイル）と一致テスト |

G フェーズは「celeris API v1 は `celeris-api-v1.md` のとおり実装済み」を前提にしてよい。ただし細部が変わりうるので、**G0 で `GET /api/v1/health` とスキーマファイルの存在を確認**する（§10.0）。

## 4. ユースケースと画面

原則 5 に従い、**受信箱を既定の画面**にする。他の画面は受信箱から辿れる補助。各画面の中身は `GET /api/v1/...` の応答をそのまま描く（派生値の規則は `celeris-api-v1.md` §5）。
...
詳細は [`celeris-api-v1.md`](celeris-api-v1.md)。要点:
...
| loopback | 認証無し（従来どおり）。ただし `CELERIS_GUI_PASSWORD_FILE` を明示したら loopback でも認証を要求する（設定したのに効かない状態を避ける。ADR-GUI-0008 D2、G5-P1） |
...
- **`.run/`（celeris の DB）はローカルディスクに置く**。`/home` などのネットワークファイルシステム（NFS）では SQLite の WAL が正しく働かず、
  ディスパッチャが 10〜30 秒止まることがある（celeris の ADR-0013 D5 / ADR-0015、`docs/celeris-requests.md` R1）。`scripts/celeris.sh` は `CELERIS_RUN_ROOT` で置き場を決め、既定はローカルディスク（G5-P5、ADR-GUI-0008 D13）。
...
- 仕様の**曖昧さ**（どちらとも読める）は、celeris の実際の挙動に合わせて進めてよい。ただし PROGRESS の「提案」に「`celeris-api-v1.md` の §x をこう明確化すべき」と書く。
...
- 受け入れ:
  ...
  5. `pnpm audit --audit-level=high` が 0 件。`pnpm release` が `dist/celeris-gui-<version>.tar.gz` を作り、空ディレクトリに展開して `pnpm install --prod --frozen-lockfile --offline --ignore-scripts && node server.js` で `/` が 200（Playwright の smoke）
...
- 目的: **「この GUI で何ができるか」が画面から分かる**ようにする。celeris の概念（タスクの状態、受け入れ条件、承認、Plan、プロバイダ、run）を
  GUI の操作と結びつけて説明する。仕様に無い機能は書かない（`docs/celeris-api-v1.md` と `docs/DESIGN.md` の範囲だけ）。
- 実装:
  - `/help` ルート（ナビゲーションに「使い方」）。SSR・CSP・a11y は他の画面と同じ規則。新しい依存は足さない。
  - 内容（この順で節を作り、`id` を付けて `/help#<id>` で飛べるようにする）:
    1. `#flow` 3 分で分かる流れ: タスクを作る → 承認 → celeris がワーカーを起動 → celeris が受け入れ条件を判定 → done（人間が触るのは承認・回答・取り消しだけ）
    2. `#screens` 画面ごとの説明: 受信箱 / 一覧 / 詳細 / DAG / プロバイダ / デーモン。「何を見る画面か」「いつ開くか」を 1〜2 文ずつ
    3. `#acceptance` 受け入れ条件の 4 種類（`command` / `artifact_exists` / `reviewer` / `human`）の使い分けと書き方の例。**判定は celeris が自分で再実行する**こと（ワーカーの自己申告は信じない）を明記
    ...
    6. `#trouble` 困ったとき: celeris が止まっているときの画面、401 / 403 / 409 / 422 の意味、run のログと成果物の見方、`docs/celeris-requests.md` に書く場面
...
- 受け入れ:
  ...
  5. 受け入れ条件・状態・用語の説明が `docs/celeris-api-v1.md` の語と一致している（§5.4 の `actions`、§3.4 の条件の型名、状態名）
  6. `pnpm lint` / `pnpm typecheck` / `pnpm test` / `pnpm build` / `pnpm e2e` が exit 0、`pnpm gen:types` の差分ゼロ

### Phase G7 — クラスタと委譲の表示（light, 50）

celeris の Phase 10（役割と委譲）と Phase 12（クラスタでのコマンド実行）で増えた情報を画面に出す。
...
- fixture: `scripts/celeris.sh fixture clusters`（`[[clusters]]` を 2 つ持つ設定。1 つは届かない host にして `connected: false` を作る）と、
...
- celeris API v1: [`celeris-api-v1.md`](celeris-api-v1.md)。提案の状態: [`celeris-proposals.md`](celeris-proposals.md)
- 一次情報の出典一覧（Remix の実体、ライブラリの版・日付・ライセンス）: ADR-GUI-0002 §2, §8
- `celeris-gui` 立ち上げ: celeris リポジトリの `docs/gui/bootstrap/README.md`
