...
} from "~/taskd/task-changes.server";
...
/**
 * 変更の取り込み（ADR-0043 D5、taskd Phase 54 / G17）。DOM を描画する unit テストが無い（G10-U1）ので、
 * 表示の判断は `~/lib/task-changes.ts` / `~/lib/labels.ts` の純粋関数、取得と送信は
 * `~/taskd/task-changes.server.ts` で見る（`test/unit/task-files.test.ts` と同じ作り）。
 */
