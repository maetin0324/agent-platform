#!/bin/sh
# scripts/celeris.sh fixture basic 用の fake ワーカー（docs/adr/0004 D5）。
...
# 拾ったり、直列化の細部が変わったときに静かに壊れる。G7-U3）。実 celeris の crate には依存しない。
# @PLAN_JSON@ は scripts/celeris.sh が絶対パスに置換する。
set -u
  ...
  *)
    case "$TITLE" in
      ...
      Failed-E)
        ...
        ;;
      Slow-F)
        # G3 受け入れ条件 7（追尾）: 2 秒おきに progress を 5 回出す（合計 10 秒）。celeris 独自プロトコルの行
      ...
      Artifacts-G)
        ...
        # `ArtifactProduced` は celeris が自動検出せず、ワーカーが `{"type":"artifact",...}` を明示的に送る必要がある
        # （docs/celeris-api-v1.md、crates/task-worker/src/protocol.rs の `WorkerMessage::Artifact`）。
