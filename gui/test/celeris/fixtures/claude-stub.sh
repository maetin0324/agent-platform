#!/bin/sh
# `claude` CLI の代わりに `[adapters.claude_code] command` へ置くスタブ（e2e/g8、scripts/celeris.sh fixture accounts）。
# celeris 側 crates/task-worker/src/claude_account.rs の実装・テスト（login_stub / check_account_ok_path_records_observation）
# と同じ形の出力をする:
