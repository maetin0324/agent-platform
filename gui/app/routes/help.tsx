...
const SCREENS: { href: string | null; icon: IconName; title: string; text: string }[] = [
  ...
  },
  {
    ...
    icon: "folder",
    title: "案件",
    text: "案件の一覧・作成と、個々の案件の途中目標・仕事の木を見る画面。新規フォームでは作業場所（手元／クラスタ／まだ決めない）も選べる。案件の詳細には「作業場所」カードがあり、現在値の確認・編集・消去ができる（未設定だと分解した仕事は空の作業ディレクトリで走る）。方針に納得したら「この方針で進める」を押すと、CoS がここまでのやり取りと途中目標をまとめて仕事に分解する（返事は待たない。仕事の木が増えていく）。途中目標は達成のたびに Go を出すか再設計する。",
  },
  {
    href: "/reports",
    icon: "send",
