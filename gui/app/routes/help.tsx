...
const GLOSSARY: { term: string; text: string }[] = [
  ...
  {
    term: "途中目標（milestone）",
    text: "SPEC §7「途中目標は予め大まかに決めておいて、適宜再設計する」。案件の中の通し番号付きの区切りで、提案 → 承認済み → 進行中 → 達成（または再設計）と進む。達成ごとに人が Go を出すか再設計するかを判定する。",
  },
  {
    term: "秘書",
...
const SCREENS: { href: string | null; icon: IconName; title: string; text: string }[] = [
  ...
  },
  {
    ...
    icon: "folder",
    title: "案件",
    text: "案件の一覧・作成と、個々の案件の途中目標・仕事の木を見る画面。方針に納得したら「この方針で進める」を押すと、秘書がここまでのやり取りと途中目標をまとめて仕事に分解する（返事は待たない。仕事の木が増えていく）。途中目標は達成のたびに Go を出すか再設計する。",
  },
  {
    href: "/reports",
    icon: "send",
