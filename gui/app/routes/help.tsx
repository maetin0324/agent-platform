...
const GLOSSARY: { term: string; text: string }[] = [
  ...
  {
    term: "口出し",
    text: "SPEC §3.4「おかしなことをしていたり、追加の指示を出したいとき、組織の木を見て『誰に言うか』を決め、その担当に直接言う。相手は『人』なので、先週の議論の続きとして話せる」。/org でノードを選んで「話す」、または案件の仕事の木のタスクから「担当に話す」。",
  },
  {
    term: "対話（messages）",
    text: "ノードごと・案件ごとに分かれたやり取り（古い順）。話しかけると裏で run が 1 回動き、その結果が返事として入る（だから返事は同期では返らず、送ったあとは「考え中」のまま待つ）。ノードの返事は Markdown として描かれ、その返事を作った run の裏方はタスク詳細（/tasks/:id）で見られる。",
  },
  {
    term: "仕事の木（DAG）",
  ...
  {
    term: "認可",
    text: "SPEC §3.6「少しでも聞くべきだとエージェントが判断したら、あなたに指示を仰ぐ」。ワーカーが run の途中で人に聞きたいことがあると質問で終わり（既存の Question 終端）、それが /approvals の「認可待ち」に 1 件として並ぶ。答えると、その答えは既存の『質問に答える』経路でタスクへ渡り、run が再開する。",
  },
  {
    term: "今回だけ／今後ずっと",
    text: "SPEC §3.6「あなたはそれに対して『今回だけ』か『同じようなことは今後ずっと』のどちらかの認可を出す」。/approvals の答えるフォームの 3 つのボタン（今回だけ・今後ずっと・認めない）に対応する。「今後ずっと」を選ぶと、その答えが規則文として永続の認可に 1 行加わる（scope でこのノードだけ／全員を選べる）。「認めない」は答えの先頭に「認めない: 」を付けてワーカーへ返す。",
  },
  {
    term: "永続の認可",
    text: "SPEC §3.6「永続の認可は文字で記録してエージェントに注入する」。standing_rules の 1 行（ノード宛て、または全員宛て）。以後、その宛先の run のプロンプトに毎回前置きされる。/approvals の下部で一覧・追加・削除ができ、質問を経ずに直接足すこともできる。組織の木（/org）のノード詳細にも、そのノードへの永続の認可が表示される。",
  },
];
...
const SCREENS: { href: string | null; icon: IconName; title: string; text: string }[] = [
  {
    ...
    icon: "message",
    title: "秘書",
    text: "SPEC §4「秘書との対話 — 案件を投げる、状況を聞く、方針を変える」。左で案件を選び（「案件なし」は雑談）、下の欄から話しかける。「新しい案件として」で送ると、その本文が新しい案件の依頼になり（先頭 40 字が案件名）、秘書が理解の確認・大まかな方針・最初の途中目標を返す（SPEC §7）。返事は同期では返らないので、送ったあとは「考え中」を出して待つ。",
  },
  {
    href: "/org",
    icon: "users",
  ...
  },
  {
    ...
    icon: "shield",
    title: "認可",
    text: "SPEC §4「認可の要求 — 聞かれたことに『今回だけ／今後ずっと』で答える。永続の認可の一覧と編集」。上に未決の要求（ノードからの質問）、その下に決めたものの履歴、さらに下に永続の認可（standing rules）の一覧・追加・削除が並ぶ。ノードの質問に「今回だけ」「今後ずっと」「認めない」のいずれかで答えると、そのタスクが再開する。「今後ずっと」で答えると、その答えが規則文として永続の認可に加わり、以後そのノード（または全員）の run のプロンプトに前置きされる。ナビの「認可」には未決の件数のバッジが付く。",
  },
  {
    href: "/artifacts",
    icon: "file",
