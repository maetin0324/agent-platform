...
export async function action({ request, params }: Route.ActionArgs) {
  ...
  // ADR-0044 D7（Phase 57 / G19）: 成果物を案件の文書に昇格する（**管理系**。宛先は人が決める）。
  if (intent === "promote") {
...
function TimelineBody({ taskId, item }: { taskId: string; item: TimelineItem }) {
  switch (item.kind) {
    ...
    // ADR-0044 D7（Phase 57 / G19）: 逆リンク。front matter の `tasks:` にこのタスクを持つページ。
    case "doc":
      return (
...
/**
 * 成果物を案件の文書に昇格する（ADR-0044 D7、docs/taskd-api-v1.md §3.89。**管理系**。Phase 57 / G19）。
