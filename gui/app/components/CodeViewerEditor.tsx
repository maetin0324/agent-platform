...
/**
 * ログ・成果物の生テキストを読み取り専用の CodeMirror 6 で表示する本体（Phase 77、ADR-0055 性能予算）。
 * `@codemirror/*`（gzip 前で 260KB 超。初回 JS 予算 350KB を単独でほぼ使い切る重さ）を静的 import するので、
 * `~/components/CodeViewer.tsx`（薄い `React.lazy` の窓口）からだけ読み込む。ここを直接 import しない。
 ...
 * 簡易フォールバックを出す。
 */
export function CodeViewerEditor({ content, json: isJson }: { content: string; json?: boolean }) {
