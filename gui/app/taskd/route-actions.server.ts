 ...
 * GUI 側で判断ロジックは書かない: どの操作が可能かは taskd（`TaskDetail.actions` / 受信箱の区画）が決め、
 * ここはフォームの値を対応する `POST` に写すだけ。409 / 422 は taskd の文言をそのまま返す。
 */
...
/** `POST /tasks`。taskd のエラーは例外にせず `CreateFailure` として返す。 */
...
/** `POST /plans`。taskd のエラーは例外にせず `CreateFailure` として返す。 */
...
/** `POST /replay`（docs/taskd-api-v1.md §3.15）。本文は空。DB は変更しない。503 `replay_in_progress` も `{ok:false}`。 */
export async function runReplay(client: TaskdClient, signal?: AbortSignal): Promise<ReplayOutcome> {
  try {
