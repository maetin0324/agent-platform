...
/**
 * 「報告」画面（`/reports`）の Discord 区画からのテスト送信（ADR-0037 D4、docs/taskd-api-v1.md §3.65。
 * **管理系**、`token_file` 未設定でも 401）。要求本文は無し。taskd のエラーは例外にせず `NotifyTestOutcome`
 * として返す（`reports-admin.server.ts` と同じ作り）。台帳（`notifications`）には残らない。
 */
export async function sendNotifyTest(client: TaskdClient, signal?: AbortSignal): Promise<NotifyTestOutcome> {
  try {
