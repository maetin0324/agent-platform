 ...
 * 依存の循環が 422 になるのは taskd の判断で、その文言をそのまま画面に出す）。
 ...
 * - `priority` は `"P1"` のラベルのまま送る（taskd が `PriorityInput` として受ける。ADR-0044 D3）
 */
 ...
 * 受け入れ条件は 1 行の自由記述を `human` の条件 1 件として送る（taskd は 1 件以上を要求する）。
 * 空欄は本文に入れない（taskd の既定に任せる。`tasks.new.tsx` の `buildNewTaskSpec` と同じ考え方）。
 */
export function buildProjectTaskSpec(form: FormData, projectId: string): NewTaskSpec {
  ...
  // `"P1"` のラベルをそのまま送る（taskd が `PriorityInput` として受ける。ADR-0044 D3）。
