...
/**
 * 「クラスタ」画面からの接続の中継（ADR-0032、docs/taskd-api-v1.md §3.39〜3.41）。
 * GUI 側では検証しない（`id` の有無だけ見る）。taskd の 404 / 409 / 422 / 502 / 401 の文言をそのまま画面に出す。
 * `accounts-admin.server.ts` のログイン中継と同じ作りだが、**`POST /reload` は呼ばない**（接続を張っても
 * `taskd.toml` の設定は変わらない。プロバイダや秘密の管理とはここが違う。`secrets-admin.server.ts` / `providers-admin.server.ts`
...
/** フォームから `id` を読む（無ければ空文字。taskd の 404 に任せる。GUI 側で検証しない）。 */
