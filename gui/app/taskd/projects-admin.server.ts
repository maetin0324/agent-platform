 ...
 * 管理系かどうかで GUI 側の中継コードは変わらない（`TaskdClient` はどちらも同じ `Authorization` ヘッダを
 * 付けるだけ。401 の案内文も `Flash.tsx` の `error.code === "unauthorized"` の 1 か所に集約されている）。
 * GUI 側では検証しない: taskd が 401 / 404 / 409 / 422 / 400 を返したらその文言をそのまま画面に出す。
 */
...
export function cancelProject(client: TaskdClient, projectId: string, signal?: AbortSignal): Promise<ProjectOpOutcome> {
...
export function pauseProject(client: TaskdClient, projectId: string, signal?: AbortSignal): Promise<ProjectOpOutcome> {
  return projectLifecycle(client, "project_pause", "pause", projectId, signal);
}

/** `POST /projects/{id}/resume`（§3.86）。`paused_from` へ戻す（戻り先を決めるのは taskd）。 */
export function resumeProject(client: TaskdClient, projectId: string, signal?: AbortSignal): Promise<ProjectOpOutcome> {
