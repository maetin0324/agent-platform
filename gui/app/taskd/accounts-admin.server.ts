...
/**
 * `/accounts` の追加・削除・確認・ログイン中継（ADR-GUI-0012 D3、docs/taskd-api-v1.md §3.29〜3.35、ADR-0025 D5/D6）。
 * GUI 側では検証しない（`id` の有無だけ見る）。taskd の 404 / 409 / 422 / 502 の文言をそのまま画面に出す。
 * `adapter` はどの呼び出しにも渡す（省略時 taskd 側の既定は `claude-code`。ADR-0025 D6）。
 */
...
/**
 * `POST /accounts/{id}/login/code`。コードはログにも応答にも出ない（taskd 側）。claude-code のみ有効
 * （codex は 409 `login_code_not_supported`、ADR-0025 D5。呼び出し自体は行い、taskd の応答をそのまま返す）。
 */
...
/** フォームから `adapter` を読む（`"codex"` 以外は `"claude-code"` にする。taskd 既定と同じ、ADR-0025 D6）。 */
