...
/**
 * `/accounts` の「API キー」節の一覧・追加/更新・削除の中継（ADR-0030 D3/D4、docs/celeris-api-v1.md §3.36〜3.38）。
 * GUI 側では検証しない: celeris が 404 / 409 / 422 / 401 を返したらその文言をそのまま画面に出す。**値は
...
export async function listSecrets(client: CelerisClient, signal?: AbortSignal): Promise<SecretList> {
  return client.get<SecretList>("/secrets", { signal });
}

/** `PUT /secrets/{id}`（作成・置き換えの両方）。空/空白の値・無効な id は celeris が 422/404 で拒否する。 */
...
/** フォームから `id` を読む（無ければ空文字。celeris の 404/422 に任せる。GUI 側で検証しない）。 */
export function readSecretId(form: FormData): string {
  return formString(form, "id") ?? "";
}

/** フォームから `value` を読む（無ければ空文字。空白だけの値は celeris が 422 で拒否する）。 */
