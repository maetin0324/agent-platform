...
/**
 * 「リリース」画面からの昇格の中継（ADR-0040 D6、docs/celeris-api-v1.md §3.67。**管理系**、
 * `token_file` 未設定でも 401）。
 *
 * GUI 側では検証しない（`sha12` の有無だけ見る）: 昇格できるかどうかは celeris と `promote.sh` が
...
/** フォームから `sha12` を読む（無ければ空文字。celeris の 404 に任せる）。 */
