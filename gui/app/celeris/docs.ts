...
/**
 * 案件の文書（ADR-0044 D7、docs/celeris-api-v1.md §3.92〜3.97。Phase 57 / G20）の読み取りの中継。
 * **正本は git**（celeris が案件の主なリポジトリの `docs/` を読む）ので、GUI は持たず・作らず・直さない。
 *
 * 文書リポジトリがまだ無い案件は celeris が 409 `docs_unavailable` を返す。ここでは例外にせず
 * `unavailable` として画面に載せ、「文書を用意する」ボタン（`POST /projects/{id}/docs/init`）を出す。
 */
export interface DocsQuery {
  /** 開いているページ（リポジトリ相対のパス）。無ければツリーだけ。 */
  path?: string | null;
  /** 検索（celeris 側は `git grep -il`）。 */
  q?: string | null;
  /** 編集の画面を開く。 */
  edit?: boolean;
}
