...
/**
 * 案件の文書の変更系（ADR-0044 D7、docs/celeris-api-v1.md §3.94〜3.97。**管理系**。Phase 57 / G20）:
 * 文書リポジトリを用意する・ページを書く・消す・成果物を昇格する。
 *
 * 規律は他の `*-admin.server.ts` と同じ: **GUI 側で検証しない**（空欄はキーごと送らない）、
 * celeris のエラーは例外にせず `{ok:false, error}` にして文言をそのまま出す。衝突（409 `etag_mismatch` /
 * `default_branch_busy` / `page_exists`）も同じ形で返るので、画面が `error.code` を見て案内する。
 */

/** `POST /projects/{id}/docs/init`（文書リポジトリを用意する）。 */
export async function initDocs(client: CelerisClient, projectId: string, signal?: AbortSignal): Promise<DocsOpOutcome> {
  try {
