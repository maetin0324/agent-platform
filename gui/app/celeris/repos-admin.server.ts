...
/**
 * 案件のリポジトリ（ADR-0043 D1、docs/celeris-api-v1.md §3.68〜3.71。Phase 52 / G16）の中継。
 ...
 * （`CelerisClient` が `Authorization` を付けるだけ。401 の案内文は `Flash.tsx` に集約されている）。
 *
 * GUI 側では検証しない（`./projects-admin.server.ts` と同じ規律）: 名前の形（slug）・重複・知らない
 * クラスタ・使用中の削除はすべて celeris が決め、422 `validation` / 409 `repo_in_use` の文言を
 * そのまま画面に出す。`is_primary` の付け替え（1 案件 1 つ）も celeris 側の規則で、GUI は再計算しない。
 */

/** `GET /projects/{id}/repos`（読み取り。primary が先頭、あとは作った順で celeris が並べる）。 */
export function listRepos(client: CelerisClient, projectId: string, signal?: AbortSignal): Promise<RepoList> {
...
/** `PATCH /repos/{id}`（管理系、200 `ProjectRepo`）。書いたものだけ変える（1 つも書かなければ celeris が 422）。 */
...
/**
 * 「主にする」（`PATCH /repos/{id}` の `is_primary: true`）。他の行の `is_primary` を落とすのは celeris 側
 ...
 * primary を消したときに残りのどれが primary になるかは celeris が決める（§3.71）。
 */
...
/**
 * `RepoCreateBody` を組む（§3.69）。**空欄はキーごと送らない**（celeris の既定に委ねる）:
 * `name` を省略するとパスの末尾から slug、`kind` を省略すると `.git` の有無で決まり、`run` を省略すると
 * `auto`。`kind` の選択肢の `auto` は「celeris に決めさせる」なので同じくキーを送らない
 ...
 * `run` は常に送り、`default_branch` は**空欄なら `null`（消す）**を明示する（`git` 以外では celeris が
