...
/**
 * 変更の取り込み（ADR-0043 D5、celeris Phase 54 / G18）の中継。
 * 読み取り（`GET /tasks/{id}/changes`、`GET /tasks/{id}/changes/{repo}/diff`）はトークン不要、
 * 取り込み（`POST .../integrate`、`POST .../pr/merge`）は**管理系（人だけ）**で、
 * どちらも同じ `CelerisClient`（`getCelerisClient()`）を通す。`Authorization` を付けるのはクライアント側で、
 * 401 の案内文は `~/components/Flash.tsx` に集約されている（`~/celeris/repos-admin.server.ts` と同じ）。
 *
 * GUI では判定も回避もしない（ADR-0043 D5 はすべて celeris 側の判断）:
 ...
 * 差分（`GET .../diff`）の失敗だけは `diffError` として画面に載せる（`~/celeris/task-files.ts` の
...
export interface TaskChangesData {
  ...
  /** 差分の取得に失敗したときの celeris の文言（400 / 403 / 404 など）。 */
 ...
 * 方法（`--merge` / `--squash` / …）は celeris の `[github] merge_method` が決める（`ChangesView.merge_method`）。
 */
 ...
 * `~/celeris/repos-admin.server.ts` と同じ規律で、GUI 側では検証しない: `confirm` を落としたら
 * celeris が 422 `validation` の `errors[0].field = "confirm"` を返す）。
 */
