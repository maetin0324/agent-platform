...
/**
 * タスクの作業ツリーの閲覧（ADR-0043 D6、docs/celeris-api-v1.md §3.72〜3.73。Phase 52 / G16）の中継。
 * **読み取りで、トークンは要らない**が、他の読み取りと同じく `CelerisClient` を通す
 * （ブラウザから celeris を直接呼ばない。DESIGN §8.1）。
 *
 * 境界（`..`・絶対パス・シンボリックリンクでの脱出）は celeris が 403 `path_forbidden` で弾き、
...
export interface TaskFilesQuery {
  /** 省略すると celeris が先頭のリポジトリ（＝ワーカーのカレントディレクトリ）を選ぶ。 */
  repo?: string | null;
...
export interface TaskFilesData {
  ...
  /** 選んだファイルの取得に失敗したときの celeris の文言（403 / 404 など）。 */
...
export async function loadTaskFiles(
  ...
  try {
    const file = await client.get<TreeFileView>(`/tasks/${encodeURIComponent(taskId)}/tree/file`, {
      // `repo` は一覧が返したものに揃える（省略時に celeris が選んだ先頭のリポジトリと同じものを読む）。
