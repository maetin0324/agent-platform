...
LIGHT_MODEL="${LIGHT_MODEL:-sonnet}"      # Phase 1,2,4,6 のメインセッション
STRONG_MODEL="${STRONG_MODEL:-opus}"      # Phase 3,5 のメインセッション。Fable が使えるなら fable
...
model_for()    { case "$1" in 3|5) echo "$STRONG_MODEL";; *) echo "$LIGHT_MODEL";; esac; }
maxturns_for() { case "$1" in 3|5) echo 60;; 4) echo 50;; *) echo 40;; esac; }
